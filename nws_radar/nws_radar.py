...
from .const import url_base, url_layer, url_radar, url_legend, url_warning
...
class Nws_Radar:
    ...
    def update(self):
        ...
        self._images_radar = self._update_mult_images(self.url_radar, self._radar_files)
        self._images_legend = self._update_mult_images(
            self.url_legend, self._legend_files
        )
        self._images_warning = self._update_mult_images(
            self.url_warning, self._warning_files
        )
    ...
    def _validate_file_list(self):
        ...
        self._warning_files = [f for f in self._warning_files if self.dist not in f]
        ...
        self._radar_files = [
            f
            for k, f in zip(radar_ts, self._radar_files)
            if k in legend_ts and k in warning_ts
        ]
        self._legend_files = [
            f
            for k, f in zip(legend_ts, self._legend_files)
            if k in radar_ts and k in warning_ts
        ]
        self._warning_files = [
            f
            for k, f in zip(warning_ts, self._warning_files)
    ...
    def image(self, outfile=None):
        ...
        b = BytesIO()
        frames = [
            self._gen_frame(radar, legend, warning)
            for radar, legend, warning in zip(
                self._images_radar, self._images_legend, self._images_warning
            )
        ]
        if frames:
